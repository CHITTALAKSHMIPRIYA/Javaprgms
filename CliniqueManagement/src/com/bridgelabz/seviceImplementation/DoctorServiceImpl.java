...
public class DoctorServiceImpl implements DoctorService {
	...
	public void showDoctorDetails() throws JsonParseException, JsonMappingException, IOException {
		...
		for (int i = 0; i < doctorlist.size(); i++) {
			System.out.println(
					Utility.mapper.writerWithDefaultPrettyPrinter().writeValueAsString(doctorlist.get(i).toString()));
		}
	}
	...
	public boolean searchByDoctorAvailability(String availability) {
		...
		boolean search = false;
		for (int i = 0; i < doctorlist.size(); i++) {
			if (doctorlist.get(i).getDoctorAvailability().equalsIgnoreCase(availability)) {
				System.out.println("doctor available");
				search = true;
			}
		}
